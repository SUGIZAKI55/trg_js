...
const UserDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get('/api/user/dashboard_data', { headers: { Authorization: `Bearer ${auth?.token}` } });
  ...
  if (loading) return <div className="text-center" style={{marginTop:'50px'}}>読み込み中...</div>;
      ...
      <div className="d-flex-center mb-5">
        <div className="card" style={{ maxWidth: '500px', width: '100%' }}>
          ...
          <div className="card-body d-flex-center">
            {Object.keys(genreStats).length > 0 ? <Radar data={radarData} options={radarOptions} /> : <p className="text-muted">データなし</p>}
          </div>
        </div>
      </div>
